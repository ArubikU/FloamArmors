...
// Optimización: usando comparaciones bitwise y fast math donde sea posible
...
// Optimización: box intersection más eficiente
bool isInside(vec3 ro, vec3 rd, vec3 size, out vec3 outNormal, bool enableTransparency) {
    ...
    return tN < 0.0; // Simplificado
}


// Optimización: Unifica lógica común en box intersection
vec3 boxIntersect(vec3 ro, vec3 rd, vec3 size, out vec3 outNormal, bool enableTransparency) {   
    ...
    // Fast normal calculation usando step en vez de conditional branches
    outNormal = -sign(rd) * step(t1.yzx, t1.xyz) * step(t1.zxy, t1.xyz);
    
    float t = tN > 0.0 ? tN : tF;
    
    // Optimización: cálculo de textura usando dot product
    ...
    // Optimización: usar mezcla en vez de if/else branch
...
// Optimización: Reescritura de la función sBoxExt para reducir código duplicado y mejorar rendimiento
vec4 sBoxExt(vec3 ro, vec3 rd, vec3 size, mat3 TBN, vec4 color, inout float T, 
    ...
    // Optimización: reducir branches con operaciones vectoriales
    bvec3 normalComp = bvec3(abs(normal.x) > 0.9, abs(normal.y) > 0.9, abs(normal.z) > 0.9);
    bool isPositive = dot(normal, normal) > 0.0; // Un truco para detectar signo sin branches
...
// Optimización: sBoxExtWithRotation - optimizada para reducir código duplicado
